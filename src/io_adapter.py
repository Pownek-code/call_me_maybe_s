...
violation -- is caught here and re-raised as InputError with a clear message,
so the engine never sees a raw FileNotFoundError / JSONDecodeError /
ValidationError.
"""
...
def load_functions(path: Path) -> List[FunctionDefinition]:
    """Parse functions_definition.json into validated
...
def write_results(path: Path, results: List[FunctionCallResult]) -> None:
    """Serialize results to the output JSON array, creating parent dirs
    as needed."""
    try:
...
def read_json_array(path: Path) -> List[object]:
    """Read a file expected to contain a JSON array.
