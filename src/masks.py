...
committed so far, produce a boolean array over the full logit width
marking which token ids are legal as the next token. The engine sets
illegal logits to -inf and argmaxes what survives.

Every mask starts from the vocab's phantom set already forbidden,
then layers the state-specific rule on top. All masks are built once
as a NumPy boolean array and applied in a single vectorized assignment
-- no Python loop walks the 151936-wide logit vector.

THE LOAD-BEARING RULE (tool-name termination):
Names can be prefixes of each other -- `fn_add` vs `fn_add_numbers`.
When the committed clean string is exactly `fn_add` and both remain viable,
we do NOT decide. We permit BOTH the continuation
char (`_`, heading to fn_add_numbers) AND the closing quote `"`
(which commits fn_add), and let the model's logits choose.
Emitting `"` is the model's signal "I meant this name";
emitting `_` means "keep going". Termination is therefore:
the model selected the closing quote while the committed string is an
exact name -- NOT merely "the committed string is a name".
...
def _base_mask(vocab: Vocabulary) -> np.ndarray:
    """A boolean array, logits-wide, True = allowed.
    Phantoms start forbidden."""
...
def tool_name_mask(
    ...
    A candidate id is allowed when EITHER:
      (a) it continues toward some allowed name:
      name.startswith(clean_str + tok); OR
      (b) clean_str is ALREADY an exact allowed name AND
    ...
    survives via (a) and `"` via (b), and the model's logits pick.
    That deferral is the termination rule.
    VECTORIZATION NOTE: the naive form tests all 151,
    936 ids against every allowed name, every generated token.
    Instead we derive the SET of legal continuation strings from the (few)
    names still matching clean_str -- a handful of strings --
...
def string_value_mask(vocab: Vocabulary, committed: str) -> np.ndarray:
    ...
    unescaped quote inside other characters.
    The lone closing quote is allowed -- it terminates the value.
    VECTORIZATION NOTE: the set of ids containing a quote never changes,
    so it is computed once and cached on the vocab object rather
    than rescanned per token.
    """
...
def _quote_bearing_ids(vocab: Vocabulary) -> np.ndarray:
    """Ids whose clean string contains a quote among other characters.
    Cached: the answer depends only on the vocabulary,
    never on generation state."""
...
def number_value_mask(vocab: Vocabulary, committed: str) -> np.ndarray:
    ...
    VECTORIZATION NOTE: numeric tokens are a small,
    fixed subset of the vocabulary (digits, '.', '-'),
    so the candidate set is computed once and cached; per step
...
def _numeric_candidate_ids(vocab: Vocabulary) -> List[int]:
    """Ids whose clean string is composed only of digits,
    '.', '-'. Cached: this depends only on the vocabulary,
    so the 152k scan happens once, not per token."""
...
def boolean_value_mask(vocab: Vocabulary, committed: str) -> np.ndarray:
    """Mask for a boolean value:
    only tokens keeping committed a prefix of 'true'
    or 'false'. The legal set is tiny,
    so it is derived directly rather than by
...
def _is_number_prefix(text: str) -> bool:
    """True if text could be extended into a valid JSON number
    (int or float)."""
    if text in ("", "-"):
        return True
...
def _is_complete_number(text: str) -> bool:
    ...
    Used by both the number mask (to decide when terminators become legal)
    and the decoder
    (to confirm a selected terminator really ends a valid value).
    """
    if text in ("", "-"):
        return False
    try:
...
# The dispatch dict: generation state -> the mask builder for that state.
# A small,
# honest mapping -- four states -- not a general parser.
