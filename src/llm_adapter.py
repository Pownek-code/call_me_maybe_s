...
tensor return types, its method names, its runtime failures -- is isolated here so
the core engine works in native Python collections and never sees the third party.
...
Any SDK exception is caught and re-raised as SDKError so no raw SDK panic leaks in.
"""
from __future__ import annotations

from typing import List

from llm_sdk import Small_LLM_Model     # correct: absolute, the installed package name
...
class LLMAdapter:
    ...
    def __init__(self, model_name: str = "Qwen/Qwen3-0.6B") -> None:
        ...
        except Exception as exc:  # noqa: BLE001 -- boundary: normalize any failure
            raise SDKError(f"failed to load model {model_name!r}: {exc}") from exc
    ...
    def logits(self, input_ids: List[int]) -> List[float]:
        """Next-token logits for a native list of ids. Native list in, native out."""
        try:
    ...
    def vocab_path(self) -> str:
        """Path to vocab.json (the real SDK method is get_path_to_vocab_file)."""
        try:
    ...
    def tokenizer_path(self) -> str:
        """Path to tokenizer.json (for the added_tokens overlay in Vocabulary)."""
        try:
    ...
    def logits_length(self) -> int:
        ...
        try:
            ...
            raise SDKError(f"could not determine logits length: {exc}") from exc
