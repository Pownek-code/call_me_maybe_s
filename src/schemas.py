...
# raise a ValidationError at parse time on anything else -- e.g. {"type": "foo"}.
# Note this is the key reason pydantic beats a frozen dataclass here: a dataclass
...
# checks at construction, so the failure lands at load, in the I/O fault domain.
...
# corruption -- the same defensive immutability the original frozen dataclass had.
...
class FunctionDefinition(BaseModel):
    """Maps the complete tool schema registry matching the tool configuration data.
    ...
    thing that HAS parameters. This is the container; ParameterProperty is the item.
    """
...
class TestPrompt(BaseModel):
    """Encapsulates an isolated query item matching the sequential batch dataset."""
...
class FunctionCallResult(BaseModel):
    """Enforces the strict structural envelope required for output serialization.

    The category distinction that makes this a SEPARATE model from
    FunctionDefinition: here `parameters` holds VALUES ({"a": 2.0, "s": "hello"}),
    whereas in FunctionDefinition it holds TYPES ({"a": {"type": "number"}}).
    One describes an instance, the other a schema. Not frozen: unlike the schema
